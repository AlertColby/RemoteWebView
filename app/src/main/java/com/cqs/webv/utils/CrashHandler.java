...
/**
 * 作者:  陈庆松
 * 创建时间: 2020\12\11 0011 11:06
 * 邮箱:chen510470614@163.com
 * 描述：该类实现了错误日志收集，将日志保存到一个文件夹，以便应用下次开启时上传日志
 */
    ...
    private String saveCrashInfo2File(Throwable ex) {
        ...
        try {
            ...
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                String path = mContext.getExternalCacheDir().getAbsolutePath()+"/CrashCollection/";
                Log.d("c_log", "path:" + path);
                File dir = new File(path);
                if (!dir.exists()) {
                    boolean isCreatedDir = dir.mkdirs();
                    Log.d("c_log", "isCreateDir: " + isCreatedDir);
                }
